...
func NewModule() *ChromeModule { return &ChromeModule{} }

func (m *ChromeModule) Name() string        { return "chrome" }
func (m *ChromeModule) Description() string { return "Google Chrome browser" }

func (m *ChromeModule) Detect() (bool, error) {
...
func (m *ChromeModule) Inventory(ctx context.Context) (*module.InventoryResult, error) {
	...
	if prof.Available && prof.Count > 0 {
		...
		result.Warnings = append(result.Warnings, "Chrome is installed but no profiles have been created yet")
	}
...
func (m *ChromeModule) Backup(ctx context.Context, opts module.BackupOptions) (*module.BackupResult, error) {
	...
	snapshot, err := archive.CreateSnapshot(opts.SnapshotsDir, m.Name(), []archive.Entry{
		{Source: basePath, ArchivePath: ".config/google-chrome"},
	})
	if err != nil {
		return nil, err
	}
...
func (m *ChromeModule) Verify(ctx context.Context, snap module.Snapshot) (*module.VerifyResult, error) {
...
func (m *ChromeModule) Doctor(ctx context.Context) (*module.DoctorResult, error) {
...
func (m *ChromeModule) Dependencies(ctx context.Context) []module.Dependency {
	return nil
}

func (m *ChromeModule) Install(ctx context.Context, opts module.RestoreOptions) error {
	return nil
}

func (m *ChromeModule) Configure(ctx context.Context, opts module.RestoreOptions) error {
	home := restoreutil.HomeDir()
	return os.MkdirAll(filepath.Join(home, ".config", "google-chrome"), 0755)
}

func (m *ChromeModule) Validate(ctx context.Context, snap module.Snapshot) (*module.ValidateResult, error) {
...
func (m *ChromeModule) Actions(ctx context.Context, snap module.Snapshot, opts module.RestoreOptions) ([]actions.Action, error) {
	home := restoreutil.HomeDir()

	return []actions.Action{
		&restoreUtilAction{
			name: "chrome_backup_config",
			desc: "Backup existing Chrome configuration",
			fn: func(ctx *runtime.RestoreContext) error {
				chromeDir := filepath.Join(home, ".config", "google-chrome")
				if !restoreutil.DirExists(chromeDir) {
					return nil
				}
				return filepath.Walk(chromeDir, func(path string, info os.FileInfo, err error) error {
					if err != nil || info.IsDir() {
						return err
					}
...
var _ actions.Provider = (*ChromeModule)(nil)
